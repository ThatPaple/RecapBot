...
function addRecap(recap, link, posted, message) {
    pool.getConnection(function (err, connection) {
        ...
        connection.query(sql, function (err, result) {
            ...
            chunks.forEach((chunk, i) => message.guild.channels.cache.get(`${config.modlogChannelID}`).send(`${message.author.tag} added to Recap :\n\`\`\`${chunk}\`\`\``));
        });
    ...
    pool.getConnection(function (err, connection) {
        connection.query('SELECT MAX(id) AS highestID, MIN(id) AS lowestID FROM recaps;', function (err, results, fields) {
            ...
            if (Number(results[0]['highestID']) < Number(id) || Number(results[0]['lowestID']) > Number(id) || Number(id) < 0) {
                message.react('⛔');
                return message.reply('The ID you have specified cannot be found within the databse.\n> Check if the specified ID is within the range.');
            }
...
module.exports = { createTables, addRecap, deleteRecap, editRecap, postRecap, truncateRecap, recallRecap, swapRecap, viewRecap, databaseEval };